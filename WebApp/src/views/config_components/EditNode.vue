  ...
  <div>
      ...
      <div style="display: inline-block; margin-left: 10px; width: 30%">       
        <form @submit.prevent="onSubmit()">
          ...
          <select class="form-control" v-model="marker.is_core" name="Type Node" id="typeNode" v-validate="'required'">
          ...
          <select class="form-control" v-model="marker.zone" name="Zone" id="Zone" v-validate="'required'">
...
export default {
  ...
  methods: {
    ...
    sendData () {                                       // send data to DB
      this.axios.get('https://fibermap-api.cmu.ac.th/editnode', {
        params: {
          id: this.marker.id,
          ...
          zone: this.marker.zone
        },
        headers: {'Authorization': sessionStorage.getItem('token')}
      })
        .then(response => {
          window.alert('Edited node success.')

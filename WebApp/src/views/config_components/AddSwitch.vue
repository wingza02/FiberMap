  ...
  <div>
      ...
      <div style="display: inline-block; margin-left: 10px; width: 30%">       
        <form @submit.prevent="onSubmit()">
          ...
          <div class="form-group" :class="{error: errors.has('Type Switch')}">
          <label for="typeSwitch">Type Switch : </label>
          <select class="form-control" v-model="isCore" name="Type Switch" id="typeSwitch" v-validate="'required'">
              <option value="0">Edge Switch</option>
              <option value="1">Core Switch</option>
          </select>
          <span style="color:red" v-show="errors.has('Type Switch')" class="alert alert-danger">{{ errors.first('Type Switch') }}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    sendData () {
      this.axios.get('https://fibermap.herokuapp.com/addswitch', {
        params: {
          ip: this.marker.ip,
        ...
        .then(response => {
          window.alert('Added switch success.')
